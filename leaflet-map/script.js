const locations = [
  ...
  { id: 3, title: "Incheon Airport", lat: 37.4602, lng: 126.4407, desc: "International airport", img: "https://airssist.com/wp-content/uploads/2023/05/Incheon-Airport-Photo-798x798.jpg", category: "Life" }
];
...
let allMarkers = [];       // all markers
let visibleMarkers = [];   // markers currently visible (after filtering)
...
function renderSidebar() {
  ...
  visibleMarkers.forEach(marker => {
    ...
    if (!parent) return; // skip markers not currently in cluster
  ...
  clusterGroups.forEach(group => {
    ...
    if (group.length === 1) {
      ...
      card.addEventListener("click", () => {
        // Only zoom & open popup; do NOT remove other markers
...
searchInput.addEventListener("input", e => {
  ...
  const filtered = allMarkers.filter(m =>
    m.options.data.title.toLowerCase().includes(keyword) ||
    m.options.data.desc.toLowerCase().includes(keyword)
  );
